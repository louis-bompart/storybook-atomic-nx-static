import { i as getRenderingRef, f as forceUpdate } from './index-53978573.js';
import { i as isInDocument } from './utils-8200d0ae.js';
...
function createAtomicCommonStore(initialStoreData) {
    ...
    const clearExistingFacetElement = (facetType, facetId) => {
        if (stencilStore.state[facetType][facetId]) {
            stencilStore.state.facetElements =
                stencilStore.state.facetElements.filter((facetElement) => facetElement.getAttribute('facet-id') !== facetId);
        }
    };
    return {
        ...stencilStore,
        ...
        unsetLoadingFlag(loadingFlag) {
            const flags = stencilStore.get('loadingFlags');
            stencilStore.set('loadingFlags', flags.filter((value) => value !== loadingFlag));
        },
        hasLoadingFlag(loadingFlag) {
            return stencilStore.get('loadingFlags').indexOf(loadingFlag) !== -1;
        },
        registerResultList(data) {
        ...
        getUniqueIDFromEngine(_engine) {
            throw new Error('getUniqueIDFromEngine not implemented at the common store level.');
        },
        getFacetElements() {
            return stencilStore
                .get('facetElements')
...
export { createAtomicCommonStore as c, isRefineModalFacet as i };

//# sourceMappingURL=store-876029fc.js.map