...
import { L as LocalizedString } from './jsx-utils-1624ef6d.js';
...
const BetweenQuotes = (props) => {
    return (h("span", { class: "font-bold truncate inline-block align-bottom max-w-full whitespace-normal", part: "highlight" },
        h(LocalizedString, { key: "between-quotations", params: { text: props.content }, bindings: props.bindings })));
};
const MagnifyingGlass = () => (h("atomic-icon", { part: "icon", icon: MagnifyingGlassIcon, class: "my-6 flex flex-col items-center w-1/2 max-w-lg" }));
const NoResults = (props) => {
    const { hasQuery, query } = props.querySummaryState;
    const content = hasQuery ? (h(LocalizedString, { bindings: props.bindings, key: "no-results-for", params: {
            query: h(BetweenQuotes, { bindings: props.bindings, content: query }),
        } })) : (props.bindings.i18n.t('no-results'));
    return (h("div", { class: "my-2 text-2xl font-medium max-w-full text-center", part: "no-results" }, content));
};
const SearchTips = (props) => {
    return (h("div", { class: "my-2 text-lg text-neutral-dark text-center", part: "search-tips" }, props.bindings.i18n.t('search-tips')));
};
const NoResultsCommon = (props, children) => {
    ...
    return [
        h("div", { class: "flex flex-col items-center h-full w-full text-on-background" },
            h(MagnifyingGlass, null),
            h(NoResults, { ...props }),
            h(SearchTips, { ...props }),
            ...children),
        h("slot", null),
...
export { NoResultsCommon as N };

//# sourceMappingURL=no-results-common-8100b672.js.map