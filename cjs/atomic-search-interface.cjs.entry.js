...
function createAtomicStore() {
    const commonStore = store.createAtomicCommonStore({
        ...
        sortOptions: [],
        iconAssetsPath: '',
        mobileBreakpoint: replaceBreakpoint.DEFAULT_MOBILE_BREAKPOINT,
        fieldsToInclude: [],
    ...
    return {
        ...
        isMobile() {
            return !window.matchMedia(searchLayout.makeDesktopQuery(commonStore.state.mobileBreakpoint)).matches;
        },
...
const AtomicSearchInterface = class {
    ...
    updateSearchConfiguration(updatedProp, newValue) {
        ...
        const { updateSearchConfiguration } = headless_esm.afe(this.engine);
    ...
    initializeWithSearchEngine(engine) {
        if (this.pipeline && this.pipeline !== engine.state.pipeline) {
            console.warn(interfaceCommon.mismatchedInterfaceAndEnginePropError('search', 'query pipeline'));
        }
        if (this.searchHub && this.searchHub !== engine.state.searchHub) {
            console.warn(interfaceCommon.mismatchedInterfaceAndEnginePropError('search', 'search hub'));
        }
    ...
    async executeFirstSearch() {
        ...
        const safeStorage = new localStorageUtils.SafeStorage();
        const standaloneSearchBoxData = safeStorage.getParsedJSON(localStorageUtils.StorageItems.STANDALONE_SEARCH_BOX_DATA, null);
        if (!standaloneSearchBoxData) {
            this.engine.executeFirstSearch();
            return;
        }
        safeStorage.removeItem(localStorageUtils.StorageItems.STANDALONE_SEARCH_BOX_DATA);
        const { updateQuery } = headless_esm.Age(this.engine);
    ...
    async getOrganizationEndpoints(organizationId, env = 'prod') {
        return headless_esm.Li(organizationId, env);
    }
    get bindings() {
        return {
            engine: this.engine,
    ...
    initFieldsToInclude() {
        const fields = headless_esm.ZA.concat(this.fieldsToInclude);
        this.store.addFieldsToInclude(fields);
    }
    registerFieldsToInclude() {
        var _a;
        (_a = this.engine) === null || _a === void 0 ? void 0 : _a.dispatch(headless_esm.jfe(this.engine).registerFieldsToInclude(this.store.state.fieldsToInclude));
    }
    ...
    initUrlManager() {
        ...
        this.urlManager = headless_esm.GO(this.engine, {
    ...
    initSearchStatus() {
        this.searchStatus = headless_esm.KO(this.engine);
...
exports.atomic_search_interface = AtomicSearchInterface;

//# sourceMappingURL=atomic-search-interface.cjs.entry.js.map