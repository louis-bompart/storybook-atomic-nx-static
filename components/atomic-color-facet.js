...
import { K as KO, j as jE, _ as _T } from './headless.esm.js';
import { A as AriaLiveRegion, F as FocusTargetController } from './accessibility-utils.js';
import { a as getFieldCaptions, g as getFieldValueCaption } from './field-utils.js';
import { I as InitializeBindings, B as BindStateToController, H as Hidden } from './initialization-utils.js';
import { M as MapProp, A as ArrayProp } from './props-utils.js';
import { p as parseDependsOn } from './depends-on.js';
import { F as FacetHeader, a as FacetContainer } from './facet-header.js';
import { F as FacetPlaceholder } from './facet-placeholder.js';
import { a as announceFacetSearchResultsWithAriaLive, F as FacetSearchInput, b as FacetSearchMatches, c as FacetShowMoreLess } from './facet-show-more-less.js';
import { s as shouldUpdateFacetSearchComponent, F as FacetValueLabelHighlight, a as shouldDisplaySearchResults } from './facet-value-label-highlight.js';
...
const ColorFacetCheckbox = (props, children) => {
    var _a;
    ...
    const partValue = (_a = props.displayValue
        .match(new RegExp('-?[_a-zA-Z]+[_a-zA-Z0-9-]*'))) === null || _a === void 0 ? void 0 : _a.toString();
    let labelRef;
    return (h("li", { key: props.displayValue, class: "relative flex items-center" },
        h("button", { id: id, role: "checkbox", part: `value-checkbox value-${partValue}${props.isSelected ? ' value-checkbox-checked' : ''}`, onClick: () => props.onClick(), onMouseDown: (e) => createRipple(e, { color: 'neutral', parent: labelRef }), "aria-checked": props.isSelected.toString(), class: `value-checkbox ${props.isSelected ? 'ring-primary' : ''}`, "aria-label": ariaLabel }),
        h("label", { ref: (ref) => (labelRef = ref), htmlFor: id, part: "value-checkbox-label", onMouseDown: (e) => createRipple(e, { color: 'neutral' }) },
            children,
...
const AtomicColorFacet$1 = /*@__PURE__*/ proxyCustomElement(class AtomicColorFacet extends HTMLElement {
    constructor() {
        super();
        this.__registerHost();
        this.__attachShadow();
    ...
    initialize() {
        this.searchStatus = KO(this.bindings.engine);
        this.facet = jE(this.bindings.engine, { options: this.facetOptions });
    ...
    initializeDependenciesManager() {
        this.dependenciesManager = _T(this.bindings.engine, {
            facetId: this.facetId,
            conditions: parseDependsOn(this.dependsOn),
        });
    }
    renderHeader() {
        return (h(FacetHeader, { i18n: this.bindings.i18n, label: this.label, onClearFilters: () => {
                this.focusTargets.headerFocus.focusAfterSearch();
                this.facet.deselectAll();
            }, numberOfActiveValues: this.numberOfSelectedValues, isCollapsed: this.isCollapsed, headingLevel: this.headingLevel, onToggleCollapse: () => (this.isCollapsed = !this.isCollapsed), headerRef: (el) => this.focusTargets.headerFocus.setTarget(el) }));
    }
    renderSearchInput() {
        if (!this.withSearch) {
            return;
        }
        return (h(FacetSearchInput, { i18n: this.bindings.i18n, label: this.label, query: this.facetState.facetSearch.query, onChange: (value) => {
                if (value === '') {
    ...
    renderValue(facetValue, onClick, isShowLessFocusTarget, isShowMoreFocusTarget) {
        ...
        switch (this.displayValuesAs) {
            case 'checkbox':
                return (h(ColorFacetCheckbox, { displayValue: displayValue, numberOfResults: facetValue.numberOfResults, isSelected: isSelected, i18n: this.bindings.i18n, onClick: onClick, searchQuery: this.facetState.facetSearch.query, buttonRef: (element) => {
                    ...
                    } }, h(FacetValueLabelHighlight, { displayValue: displayValue, isSelected: isSelected, searchQuery: this.facetState.facetSearch.query })));
            case 'box':
                return (h(FacetValueBox, { displayValue: displayValue, numberOfResults: facetValue.numberOfResults, isSelected: isSelected, i18n: this.bindings.i18n, onClick: onClick, searchQuery: this.facetState.facetSearch.query, buttonRef: (element) => {
                        isShowLessFocusTarget &&
                            this.focusTargets.showLessFocus.setTarget(element);
                        isShowMoreFocusTarget &&
                            this.focusTargets.showMoreFocus.setTarget(element);
                    } }, h("div", { part: `value-${partValueWithDisplayValue} value-${partValueWithAPIValue} default-color-value`, class: "value-box-color w-full h-12 bg-neutral-dark rounded-md mb-2" }), h(FacetValueLabelHighlight, { displayValue: displayValue, isSelected: isSelected, isExcluded: isExcluded, searchQuery: this.facetState.facetSearch.query })));
        }
    }
    ...
    renderValues() {
        return this.renderValuesContainer(this.facetState.values.map((value, i) => this.renderValue(value, () => this.facet.toggleSelect(value), i === 0, i ===
            (this.sortCriteria === 'automatic'
                ? 0
                : this.resultIndexToFocusOnShowMore))));
    }
    renderSearchResults() {
        return this.renderValuesContainer(this.facetState.facetSearch.values.map((value) => this.renderValue({
            state: 'idle',
            numberOfResults: value.count,
            value: value.rawValue,
        }, () => this.facet.facetSearch.select(value), false, false)), this.facetState.facetSearch.query);
    }
    renderMatches() {
        return (h(FacetSearchMatches, { i18n: this.bindings.i18n, query: this.facetState.facetSearch.query, numberOfMatches: this.facetState.facetSearch.values.length, hasMoreMatches: this.facetState.facetSearch.moreValuesAvailable }));
    }
    renderShowMoreLess() {
    ...
    render() {
        ...
        if (!this.searchStatusState.firstSearchExecuted) {
            return (h(FacetPlaceholder, { numberOfValues: this.numberOfValues, isCollapsed: this.isCollapsed }));
        }
        if (!this.facetState.values.length) {
            return h(Hidden, null);
        }
        return (h(FacetContainer, null, this.renderHeader(), !this.isCollapsed && [
    ...
    static get style() { return AtomicColorFacetStyle0; }
}, [1, "atomic-color-facet", {
        "facetId": [1537, "facet-id"],
        "label": [513],
        "field": [513],
        "numberOfValues": [514, "number-of-values"],
        "withSearch": [516, "with-search"],
        ...
        "injectionDepth": [514, "injection-depth"],
        "dependsOn": [16],
        "allowedValues": [1025, "allowed-values"],
        "customSort": [1025, "custom-sort"],
        "facetState": [32],
...
__decorate([
    InitializeBindings()
], AtomicColorFacet$1.prototype, "bindings", void 0);
__decorate([
    BindStateToController('facet')
], AtomicColorFacet$1.prototype, "facetState", void 0);
__decorate([
    BindStateToController('searchStatus')
], AtomicColorFacet$1.prototype, "searchStatusState", void 0);
__decorate([
    MapProp()
], AtomicColorFacet$1.prototype, "dependsOn", void 0);
__decorate([
    ArrayProp()
], AtomicColorFacet$1.prototype, "allowedValues", void 0);
__decorate([
    ArrayProp()
], AtomicColorFacet$1.prototype, "customSort", void 0);
__decorate([
    AriaLiveRegion('facet-search')
], AtomicColorFacet$1.prototype, "facetSearchAriaMessage", void 0);
function defineCustomElement$1() {
    if (typeof customElements === "undefined") {
        return;
    }
    const components = ["atomic-color-facet", "atomic-component-error", "atomic-icon"];
    components.forEach(tagName => { switch (tagName) {
        case "atomic-color-facet":
            if (!customElements.get(tagName)) {
                customElements.define(tagName, AtomicColorFacet$1);
            }
            break;
...
const AtomicColorFacet = AtomicColorFacet$1;
const defineCustomElement = defineCustomElement$1;

export { AtomicColorFacet, defineCustomElement };

//# sourceMappingURL=atomic-color-facet.js.map