...
const atomicRating = require('./atomic-rating-84c9ec16.js');
...
const facetValueCheckbox = require('./facet-value-checkbox-a15e4807.js');
...
const AtomicRatingFacet = class {
    ...
    initializeFacet() {
        ...
        this.facet = headless_esm.eI(this.bindings.engine, { options });
    ...
    generateCurrentValues() {
        ...
        for (let i = this.minValueInIndex; i <= this.numberOfIntervals; i++) {
            currentValues.push(headless_esm.xs({
    ...
    ratingContent(facetValue) {
        return (index.h(atomicRating.Rating, { i18n: this.bindings.i18n, numberOfTotalIcons: this.maxValueInIndex, numberOfActiveIcons: facetValue.start, icon: this.icon }));
    }
    renderHeader() {
    ...
    renderValue(facetValue, onClick) {
        ...
        switch (this.displayValuesAs) {
            case 'checkbox':
                return (index.h(facetValueCheckbox.FacetValueCheckbox, { displayValue: displayValue, numberOfResults: facetValue.numberOfResults, isSelected: isSelected, i18n: this.bindings.i18n, onClick: onClick }, this.ratingContent(facetValue)));
            case 'link':
                return (index.h(facetValueLink.FacetValueLink, { displayValue: displayValue, numberOfResults: facetValue.numberOfResults, isSelected: isSelected, i18n: this.bindings.i18n, onClick: onClick }, this.ratingContent(facetValue)));
        }
    }
    renderValuesContainer(children) {
        return (index.h(facetValuesGroup.FacetValuesGroup, { i18n: this.bindings.i18n, label: this.label }, index.h("ul", { class: "mt-3", part: "values" }, children)));
    }
    renderValues() {
    ...
    render() {
        ...
        if (!this.searchStatusState.firstSearchExecuted) {
            return (index.h(facetPlaceholder.FacetPlaceholder, { numberOfValues: this.numberOfIntervals, isCollapsed: this.isCollapsed }));
        }
        if (!this.valuesToRender.length) {
            return index.h(initializationUtils.Hidden, null);
        }
        return (index.h(facetPlaceholder.FacetContainer, null, this.renderHeader(), !this.isCollapsed && this.renderValues()));
    }
...
AtomicRatingFacet.style = AtomicRatingFacetStyle0;

exports.atomic_rating_facet = AtomicRatingFacet;

//# sourceMappingURL=atomic-rating-facet.cjs.entry.js.map