...
const ClearButton = ({ inputRef, bindings, onClick, ...defaultButtonProps }) => (index.h(button.Button, { style: "text-transparent", part: "clear-button", class: "w-8 h-8 mr-1.5 text-neutral-dark shrink-0", onClick: () => {
        onClick === null || onClick === void 0 ? void 0 : onClick();
        inputRef === null || inputRef === void 0 ? void 0 : inputRef.focus();
    }, ariaLabel: bindings.i18n.t('clear'), ...defaultButtonProps },
    index.h("atomic-icon", { part: "clear-icon", icon: clear.ClearIcon, class: "w-3 h-3" })));
...
const AtomicSearchBox = class {
    constructor(hostRef) {
        ...
        this.redirect = index.createEvent(this, "redirect", 7);
         ...
         */
        this.textarea = false;
        this.updateBreakpoints = utils.once(() => replaceBreakpoint.updateBreakpoints(this.host));
        this.renderTextBox = (searchLabel) => {
            ...
            return this.textarea ? (index.h(searchSuggestion.SearchTextArea, { textAreaRef: this.textAreaRef, ref: (el) => (this.textAreaRef = el), ...props, onClear: () => {
                    props.onClear();
                    this.triggerTextAreaChange('');
                } })) : (index.h(SearchInput, { inputRef: this.inputRef, ref: (el) => (this.inputRef = el), ...props }));
        };
    ...
    initialize() {
        this.id = utils.randomID('atomic-search-box-');
        if (!this.textarea) {
            this.bindings.engine.logger.warn('As of Atomic version 3.0.0, the searchbox will be enabled as a text area by default. To remove this warning, set textarea="true" on the search box.', this.host);
        }
        this.searchBox = this.redirectionUrl
            ? headless_esm.iO(this.bindings.engine, {
                options: {
                    ...this.searchBoxOptions,
                    redirectionUrl: this.redirectionUrl,
                },
            })
            : headless_esm.jm(this.bindings.engine, {
    ...
    componentWillUpdate() {
        ...
        const storage = new localStorageUtils.SafeStorage();
        storage.setJSON(localStorageUtils.StorageItems.STANDALONE_SEARCH_BOX_DATA, data);
    ...
    initializeSuggestionManager() {
        ...
        this.suggestionManager = new searchSuggestion.SuggestionManager({
    ...
    renderSuggestion(item, index$1, lastIndex, side) {
        const id = `${this.id}-${side}-suggestion-${item.key}`;
        const isSelected = id === this.suggestionManager.activeDescendant;
        if (index$1 === lastIndex && item.hideIfLast) {
            return null;
        }
        const isButton = item.onSelect || item.query;
        if (!isButton) {
            return (index.h(searchSuggestion.SimpleSearchSuggestion, { bindings: this.bindings, id: id, suggestion: item, isSelected: isSelected, side: side, index: index$1, lastIndex: lastIndex, isDoubleList: this.suggestionManager.isDoubleList }));
        }
        return (index.h(searchSuggestion.ButtonSearchSuggestion, { bindings: this.bindings, id: id, suggestion: item, isSelected: isSelected, side: side, index: index$1, lastIndex: lastIndex, isDoubleList: this.suggestionManager.isDoubleList, onClick: (e) => {
                var _a;
    ...
    renderPanel(side, elements, setRef, getRef) {
        ...
        return (index.h("div", { part: `suggestions suggestions-${side}`, ref: setRef, class: "flex flex-grow basis-1/2 flex-col", onMouseDown: (e) => {
                if (e.target === getRef()) {
                    e.preventDefault();
                }
    ...
    get shouldShowSuggestions() {
        return (this.suggestionManager.hasSuggestions &&
            this.isExpanded &&
            !this.isSearchDisabledForEndUser(this.searchBoxState.value));
    }
    renderSuggestions() {
        return (index.h("div", { id: `${this.id}-popup`, part: `suggestions-wrapper ${this.suggestionManager.isDoubleList
                ? 'suggestions-double-list'
                : 'suggestions-single-list'}`, class: `flex w-full z-10 absolute left-0 top-full rounded-md bg-background border border-neutral ${this.shouldShowSuggestions ? '' : 'hidden'}`, role: "application", "aria-label": this.bindings.i18n.t(this.suggestionManager.isDoubleList
                ? 'search-suggestions-double-list'
                : 'search-suggestions-single-list'), ...(this.suggestionManager.activeDescendant && {
    ...
    renderAbsolutePositionSpacer() {
        return (index.h("textarea", { "aria-hidden": true, part: "textarea-spacer", class: "invisible text-lg py-3.5 px-4 w-full", rows: 1 }));
    }
    isSearchDisabledForEndUser(queryValue) {
        if (bueno_esm.isNullOrUndefined(queryValue)) {
    ...
    announceNewActiveSuggestionToScreenReader() {
        ...
        if (deviceUtils.isMacOS() && ariaLabel) {
            this.suggestionsAriaMessage = ariaLabel;
        }
    }
    announceNewSuggestionsToScreenReader() {
        const elsLength = this.suggestionManager.allSuggestionElements.filter(suggestionsCommon.elementHasQuery).length;
...
__decorate([
    initializationUtils.BindStateToController('searchBox')
], AtomicSearchBox.prototype, "searchBoxState", void 0);
__decorate([
    accessibilityUtils.AriaLiveRegion('search-box')
], AtomicSearchBox.prototype, "searchBoxAriaMessage", void 0);
__decorate([
    accessibilityUtils.AriaLiveRegion('search-suggestions', true)
], AtomicSearchBox.prototype, "suggestionsAriaMessage", void 0);
AtomicSearchBox.style = AtomicSearchBoxStyle0;

exports.atomic_search_box = AtomicSearchBox;

//# sourceMappingURL=atomic-search-box.cjs.entry.js.map