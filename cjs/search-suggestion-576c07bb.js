...
const SearchBoxWrapper = (props, children) => {
    ...
    return (index.h("div", { part: "wrapper", class: getClasses() }, children));
};
...
const TextAreaClearButton = ({ inputRef, bindings, onClick, ...defaultButtonProps }) => (index.h("div", { part: "clear-button-wrapper", class: "py-2 flex items-start justify-center items-center ml-2" },
    index.h(button.Button, { style: "text-transparent", part: "clear-button", class: "flex items-center justify-center w-8 h-8 text-neutral-dark shrink-0", onClick: () => {
            onClick === null || onClick === void 0 ? void 0 : onClick();
            inputRef === null || inputRef === void 0 ? void 0 : inputRef.focus();
        }, ariaLabel: bindings.i18n.t('clear'), ...defaultButtonProps },
        index.h("atomic-icon", { part: "clear-icon", icon: ClearSlim, class: "w-4 h-4" }))));
...
const SearchTextArea = ({ textAreaRef, loading, bindings, onInput, onFocus, onBlur, onChange, onKeyDown, onKeyUp, value, ariaLabel, onClear, popup, ...defaultInputProps }) => (index.h("div", { class: "grow flex overflow-hidden" },
    index.h("div", { part: "textarea-expander", class: "grow grid overflow-hidden" },
        index.h("textarea", { part: "textarea", "aria-label": ariaLabel, placeholder: bindings.i18n.t('search'), class: "placeholder-neutral-dark", rows: 1, onInput: (e) => {
    ...
    loading && (index.h("div", { class: "searchbox-button-wrapper flex items-center justify-center ml-2" },
        index.h("span", { part: "loading", class: "loading w-5 h-5 rounded-full bg-gradient-to-r animate-spin mr-2 grid place-items-center" }))),
    !loading && value && (index.h(TextAreaClearButton, { inputRef: textAreaRef, bindings: bindings, onClick: () => {
            onClear();
...
class SuggestionManager {
    constructor(ownerSearchBoxProps) {
        ...
        this.triggerSuggestions = debounceUtils.debounce(() => this.executeQuerySuggestion(), this.ownerSearchBoxProps.getSuggestionDelay());
    }
    ...
    updateActiveDescendant(id = '') {
        ...
        index.forceUpdate(this.ownerSearchBoxProps.getHost());
    }
    ...
    isRightPanelInFocus() {
        if (bueno_esm.isNullOrUndefined(this.panelInFocus) ||
            bueno_esm.isNullOrUndefined(this.rightPanel)) {
            return false;
        }
    ...
    async executeQuerySuggestion() {
        ...
        const defaultSuggestedQuery = ((_a = this.allSuggestionElements.find(suggestionsCommon.elementHasQuery)) === null || _a === void 0 ? void 0 : _a.query) || '';
    ...
    isPanelInFocus(panel, query) {
        ...
        if (query) {
            const escaped = utils.DOMPurify.sanitize(query);
    ...
    getAndFilterLeftSuggestionElements() {
        ...
        const out = suggestionElements.filter((suggestionElement) => {
            if (bueno_esm.isNullOrUndefined(suggestionElement.query)) {
                return true;
            }
    ...
    getSuggestionElements(suggestions) {
        ...
        const max = this.ownerSearchBoxProps.getNumberOfSuggestionsToDisplay() +
            elements.filter(suggestionsCommon.elementHasNoQuery).length;
...
function getContentForSuggestion(suggestion) {
    return !isHTMLElement(suggestion.content) ? (suggestion.content) : (index.h(index.Fragment, null));
}
