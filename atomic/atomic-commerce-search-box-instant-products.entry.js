...
import { k as buildInstantProducts } from './headless.esm-628c1e39.js';
...
const AtomicCommerceSearchBoxInstantProducts = class {
    ...
    renderItems() {
        ...
        const elements = products.map((product) => {
            ...
            return {
                ...partialItem,
                content: (h("atomic-product", { key: `instant-product-${encodeForDomAttribute(product.permanentid)}`, part: "outline", product: product, interactiveProduct: buildInteractiveInstantProduct(this.bindings.engine, {
                        options: { product },
                    }), display: this.display, density: this.density, imageSize: this.imageSize, content: this.itemTemplateProvider.getTemplateContent(product), stopPropagation: false, renderingFunction: this.itemRenderingFunction })),
                onSelect: (e) => {
