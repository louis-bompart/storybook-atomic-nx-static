...
const AtomicBreadbox = class {
    constructor(hostRef) {
         ...
         */
        this.pathLimit = 3;
        this.breadcrumbManagerState = undefined;
    ...
    validateProps() {
        new bueno_esm.Schema({
            pathLimit: new bueno_esm.NumberValue({
                default: 3,
                min: 1,
                required: false,
            }),
    ...
    get focusTargets() {
        if (!this.breadcrumbRemovedFocus) {
            this.breadcrumbRemovedFocus = new accessibilityUtils.FocusTargetController(this);
        }
        if (!this.breadcrumbShowLessFocus) {
            this.breadcrumbShowLessFocus = new accessibilityUtils.FocusTargetController(this);
        }
        if (!this.breadcrumbShowMoreFocus) {
            this.breadcrumbShowMoreFocus = new accessibilityUtils.FocusTargetController(this);
        }
        return {
    ...
    renderBreadcrumb(breadcrumb, index$1, totalBreadcrumbs) {
        var _a;
        const classNames = [
            'py-2',
            'px-3',
        ...
        return (index.h("li", { class: "breadcrumb", key: value }, index.h(button.Button, { part: "breadcrumb-button", style: isExclusion ? 'outline-error' : 'outline-bg-neutral', class: classNames.join(' '), title: title,
            // TODO: [KIT-2939] Replace `remove-filter-on` by `remove-include-filter-on`.
            ariaLabel: this.bindings.i18n.t(isExclusion ? 'remove-exclusion-filter-on' : 'remove-filter-on', {
                value: title,
            }), onClick: () => {
                var _a;
                if (isLastBreadcrumb) {
                    (_a = this.bindings.store.state.resultList) === null || _a === void 0 ? void 0 : _a.focusOnFirstResultAfterNextSearch();
                }
                else if (this.numberOfBreadcrumbs > 1) {
                    this.focusTargets.breadcrumbRemovedFocus.focusAfterSearch();
                }
                this.lastRemovedBreadcrumbIndex = index$1;
                breadcrumb.deselect();
            }, ref: (ref) => {
                if (this.lastRemovedBreadcrumbIndex === index$1) {
                    this.focusTargets.breadcrumbRemovedFocus.setTarget(ref);
                }
                if (this.firstExpandedBreadcrumbIndex === index$1) {
                    this.focusTargets.breadcrumbShowMoreFocus.setTarget(ref);
                }
            } }, index.h("span", { part: "breadcrumb-label", class: `max-w-snippet truncate group-hover:text-${activeColor} group-focus-visible:text-${activeColor} ${breadcrumb.state}` }, this.bindings.i18n.t('with-colon', { text: breadcrumb.label })), index.h("span", { part: "breadcrumb-value", class: breadcrumb.content
                ? ''
                : `max-w-snippet truncate ${breadcrumb.state}` }, (_a = breadcrumb.content) !== null && _a !== void 0 ? _a : value), index.h("atomic-icon", { part: "breadcrumb-clear", class: "w-2.5 h-2.5 ml-2 mt-px", icon: close.CloseIcon }))));
    }
    updateShowMoreValue(value) {
        ...
        this.showMore.textContent = `+ ${value.toLocaleString(this.bindings.i18n.language)}`;
        this.showMore.setAttribute('aria-label', this.bindings.i18n.t('show-n-more-filters', {
            value,
        }));
    }
    renderShowMore() {
        return (index.h("li", { key: "show-more" }, index.h(button.Button, { ref: (ref) => {
                this.focusTargets.breadcrumbShowLessFocus.setTarget(ref);
                this.showMore = ref;
            }, part: "show-more", style: "outline-primary", class: "p-2 btn-pill whitespace-nowrap", onClick: () => {
    ...
    renderShowLess() {
        return (index.h("li", { key: "show-less" }, index.h(button.Button, { ref: (ref) => (this.showLess = ref), part: "show-less", style: "outline-primary", text: this.bindings.i18n.t('show-less'), class: "p-2 btn-pill", onClick: () => {
    ...
    renderClearAll() {
        ...
        return (index.h("li", { key: "clear-all" }, index.h(button.Button, { part: "clear", style: "text-primary", text: this.bindings.i18n.t('clear'), class: "p-2 btn-pill", ariaLabel: this.bindings.i18n.t('clear-all-filters'), onClick: async () => {
                var _a;
    ...
    get facetBreadcrumbs() {
        return this.breadcrumbManagerState.facetBreadcrumbs
            .map(({ facetId, field, values }) => values.map((value) => ({ value, facetId, field })))
            .flat()
            ...
            return ({
                facetId,
                label: (_a = this.bindings.store.state.facets[facetId]) === null || _a === void 0 ? void 0 : _a.label(),
                state: value.value.state,
                deselect: value.deselect,
                formattedValue: [
                    fieldUtils.getFieldValueCaption(field, value.value.value, this.bindings.i18n),
                ],
            });
        });
    }
    get categoryFacetBreadcrumbs() {
        return this.breadcrumbManagerState.categoryFacetBreadcrumbs.map(({ facetId, field, path, deselect }) => ({
            facetId,
            label: this.bindings.store.state.categoryFacets[facetId].label(),
            deselect: deselect,
            formattedValue: path.map((pathValue) => fieldUtils.getFieldValueCaption(field, pathValue.value, this.bindings.i18n)),
        }));
    }
    get numericFacetBreadcrumbs() {
        return this.breadcrumbManagerState.numericFacetBreadcrumbs
            ...
            return ({
                ...
                content: (_b = (_a = this.bindings.store.state.numericFacets[facetId]).content) === null || _b === void 0 ? void 0 : _b.call(_a, value.value),
            });
        });
    }
    ...
    get automaticFacetBreadcrumbs() {
        return this.breadcrumbManagerState.automaticFacetBreadcrumbs
            .flatMap(({ facetId, field, label, values }) => values.map((value) => ({ value, facetId, field, label })))
    ...
    renderBreadcrumbs(allBreadcrumbs) {
        ...
        return [
            sortedBreadcrumbs.map((breadcrumb, i) => this.renderBreadcrumb(breadcrumb, i, allBreadcrumbs.length)),
    ...
    render() {
        ...
        return (index.h("div", { part: "container", class: "text-on-background text-sm flex" }, index.h("span", { part: "label", class: "font-bold py-[0.625rem] pl-0 pr-2" }, this.bindings.i18n.t('with-colon', {
            text: this.bindings.i18n.t('filters'),
        })), index.h("div", { part: "breadcrumb-list-container", class: "relative grow" }, index.h("ul", { part: "breadcrumb-list", class: `flex gap-1 ${this.isCollapsed ? 'flex-nowrap absolute w-full' : 'flex-wrap'}` }, this.renderBreadcrumbs(allBreadcrumbs)))));
    }
    componentDidRender() {
...
], AtomicBreadbox.prototype, "bindings", void 0);
__decorate([
    initializationUtils.BindStateToController('breadcrumbManager')
], AtomicBreadbox.prototype, "breadcrumbManagerState", void 0);
__decorate([
    initializationUtils.BindStateToController('facetManager')
], AtomicBreadbox.prototype, "facetManagerState", void 0);
AtomicBreadbox.style = AtomicBreadboxStyle0;

exports.atomic_breadbox = AtomicBreadbox;

//# sourceMappingURL=atomic-breadbox.cjs.entry.js.map