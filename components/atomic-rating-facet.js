...
import { K as KO, i as eI, _ as _T, x as xs } from './headless.esm.js';
import { S as Star, R as Rating } from './atomic-rating.js';
...
const AtomicRatingFacet$1 = /*@__PURE__*/ proxyCustomElement(class AtomicRatingFacet extends HTMLElement {
    constructor() {
        super();
        this.__registerHost();
        this.__attachShadow();
    ...
    initializeFacet() {
        ...
        this.facet = eI(this.bindings.engine, { options });
    ...
    ratingContent(facetValue) {
        return (h(Rating, { i18n: this.bindings.i18n, numberOfTotalIcons: this.maxValueInIndex, numberOfActiveIcons: facetValue.start, icon: this.icon }));
    }
    renderHeader() {
    ...
    renderValue(facetValue, onClick) {
        const displayValue = this.formatFacetValue(facetValue);
        const isSelected = facetValue.state === 'selected';
        switch (this.displayValuesAs) {
            case 'checkbox':
                return (h(FacetValueCheckbox, { displayValue: displayValue, numberOfResults: facetValue.numberOfResults, isSelected: isSelected, i18n: this.bindings.i18n, onClick: onClick }, this.ratingContent(facetValue)));
            case 'link':
                return (h(FacetValueLink, { displayValue: displayValue, numberOfResults: facetValue.numberOfResults, isSelected: isSelected, i18n: this.bindings.i18n, onClick: onClick }, this.ratingContent(facetValue)));
        }
    }
    renderValuesContainer(children) {
        return (h(FacetValuesGroup, { i18n: this.bindings.i18n, label: this.label }, h("ul", { class: "mt-3", part: "values" }, children)));
    }
    renderValues() {
        return this.renderValuesContainer(this.valuesToRender.map((value) => this.renderValue(value, () => this.displayValuesAs === 'link'
            ? this.facet.toggleSingleSelect(value)
            : this.facet.toggleSelect(value))));
    }
    get valuesToRender() {
        return this.facet.state.values.filter((value) => value.numberOfResults || value.state !== 'idle');
    }
    render() {
        ...
        if (!this.searchStatusState.firstSearchExecuted) {
            return (h(FacetPlaceholder, { numberOfValues: this.numberOfIntervals, isCollapsed: this.isCollapsed }));
        }
        if (!this.valuesToRender.length) {
            return h(Hidden, null);
        }
        return (h(FacetContainer, null, this.renderHeader(), !this.isCollapsed && this.renderValues()));
    }
    get host() { return this; }
    static get style() { return AtomicRatingFacetStyle0; }
}, [1, "atomic-rating-facet", {
        "facetId": [1537, "facet-id"],
        "label": [513],
        "field": [513],
        "numberOfIntervals": [514, "number-of-intervals"],
        "maxValueInIndex": [514, "max-value-in-index"],
        "minValueInIndex": [514, "min-value-in-index"],
...
__decorate([
    InitializeBindings()
], AtomicRatingFacet$1.prototype, "bindings", void 0);
__decorate([
    BindStateToController('facet')
], AtomicRatingFacet$1.prototype, "facetState", void 0);
__decorate([
    BindStateToController('searchStatus')
], AtomicRatingFacet$1.prototype, "searchStatusState", void 0);
__decorate([
    MapProp()
], AtomicRatingFacet$1.prototype, "dependsOn", void 0);
function defineCustomElement$1() {
    if (typeof customElements === "undefined") {
        return;
    }
    const components = ["atomic-rating-facet", "atomic-component-error", "atomic-icon"];
    components.forEach(tagName => { switch (tagName) {
        case "atomic-rating-facet":
            if (!customElements.get(tagName)) {
                customElements.define(tagName, AtomicRatingFacet$1);
            }
            break;
...
const AtomicRatingFacet = AtomicRatingFacet$1;
const defineCustomElement = defineCustomElement$1;

export { AtomicRatingFacet, defineCustomElement };

//# sourceMappingURL=atomic-rating-facet.js.map