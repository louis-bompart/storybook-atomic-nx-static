...
const QuickviewSidebar = (props) => {
    ...
    const minimizeButton = (index.h(MinimizeButton, { ...props, wordsLength: numberOfWords }));
    return (index.h("div", { class: "p-4 border-r border-neutral h-full" },
        minimized && minimizeButton,
        index.h("div", { class: "flex items-center justify-between" },
            index.h("div", { class: "flex items-center" },
                index.h(HighlightKeywordsCheckbox, { ...props })),
            !minimized && index.h("div", null, minimizeButton)),
        !minimized && index.h(Keywords, { ...props, words: words })));
};
const MinimizeButton = ({ i18n, minimized, onMinimize, highlightKeywords, wordsLength }) => (index.h(iconButton.IconButton, { partPrefix: "sidebar-minimize", icon: MinimizeIcon, style: "text-transparent", title: i18n.t('quickview-toggle-navigation'), ariaLabel: i18n.t('quickview-toggle-navigation'), onClick: () => onMinimize(!minimized), badge: highlightKeywords && minimized ? index.h("slot", null, wordsLength) : undefined, class: `w-fit ${minimized ? '' : 'ml-auto'}`, ariaExpanded: (!minimized).toString(), ariaControls: identifierKeywordsSection }));
const HighlightKeywordsCheckbox = ({ i18n, highlightKeywords, onHighlightKeywords, minimized }) => (index.h(index.Fragment, null,
    index.h(checkbox.Checkbox, { text: i18n.t('keywords-highlight'), class: "mr-2", id: "atomic-quickview-sidebar-highlight-keywords", checked: !highlightKeywords.highlightNone, onToggle: (checked) => onHighlightKeywords({
            ...highlightKeywords,
            highlightNone: !checked,
        }) }),
    !minimized && (index.h("label", { class: "font-bold cursor-pointer whitespace-nowrap", htmlFor: "atomic-quickview-sidebar-highlight-keywords" }, i18n.t('keywords-highlight')))));
const Keywords = ({ words, i18n, highlightKeywords, onHighlightKeywords }) => {
    return (index.h("div", { id: identifierKeywordsSection }, Object.values(words).map((keyword) => {
        ...
        return (index.h("div", { key: keyword.text, class: "flex items-center justify-between gap-x-2 my-4 w-100" },
            index.h("div", { class: `flex grow items-center bg-background border border-neutral rounded-lg overflow-x-auto ${!wordIsEnabled ? 'pointer-events-none opacity-50' : ''}` },
                index.h("div", { class: "flex items-center grow p-4 border-r", "aria-hidden": "true" },
                    index.h("div", { class: "w-5 h-5 flex-none mr-2", style: { backgroundColor: keyword.color } }),
                    index.h("div", { class: "grow mr-2 whitespace-nowrap" }, keyword.text),
                    index.h("div", { class: "flex-none" },
                        ...
                        ")")),
                index.h(fieldsetGroup.FieldsetGroup, { label: i18n.t('quickview-navigate-keywords', {
                        occurrences: keyword.occurrences,
                        keyword: keyword.text,
                    }) },
                    index.h("div", { class: "flex px-2" },
                        index.h(iconButton.IconButton, { partPrefix: "sidebar-next", icon: arrowBottomRounded.ArrowBottomIcon, disabled: !wordIsEnabled, style: "text-transparent", class: "border-0", ariaLabel: i18n.t('next'), title: i18n.t('next'), onClick: () => keyword.navigateForward() }),
                        index.h(iconButton.IconButton, { partPrefix: "sidebar-previous", icon: arrowTopRounded.ArrowUp, disabled: !wordIsEnabled, style: "text-transparent", class: "border-0", ariaLabel: i18n.t('previous'), title: i18n.t('previous'), onClick: () => keyword.navigateBackward() })))),
            index.h(iconButton.IconButton, { partPrefix: "sidebar-remove-word", class: `${highlightKeywords.highlightNone
...
const QuickviewIframe = ({ onSetIframeRef, uniqueIdentifier, content, sandbox, src, logger }) => {
    ...
    return (index.h("iframe", { src: "about:blank", class: "w-full h-full", sandbox: sandbox, ref: async (el) => {
            const iframeRef = el;
...
const AtomicQuickviewModal = class {
    constructor(hostRef) {
        index.registerInstance(this, hostRef);
        this.nextQuickview = index.createEvent(this, "atomic/quickview/next", 7);
        this.previousQuickview = index.createEvent(this, "atomic/quickview/previous", 7);
    ...
    renderHeader() {
        ...
        if (this.result) {
            ...
            headerContent = (index.h(index.Fragment, null, index.h(itemLink.LinkWithItemAnalytics, { href: (_a = this.result) === null || _a === void 0 ? void 0 : _a.clickUri, onSelect: () => { var _a; return (_a = this.interactiveResult) === null || _a === void 0 ? void 0 : _a.select(); }, onBeginDelayedSelect: () => { var _a; return (_a = this.interactiveResult) === null || _a === void 0 ? void 0 : _a.beginDelayedSelect(); }, onCancelPendingSelect: () => { var _a; return (_a = this.interactiveResult) === null || _a === void 0 ? void 0 : _a.cancelPendingSelect(); }, className: "truncate", part: "quickview-modal-header-title" }, this.result.title), index.h(iconButton.IconButton, { partPrefix: "quickview-modal-header", icon: close.CloseIcon, onClick: () => this.onClose(), ariaLabel: this.bindings.i18n.t('close'), style: "text-transparent", title: this.bindings.i18n.t('close') })));
        }
        return (index.h("div", { slot: "header", class: "w-full flex justify-between items-center" }, headerContent));
    }
    renderBody() {
        return (index.h("div", { slot: "body", class: "grid grid-cols-[min-content_auto] h-full" }, index.h("div", { class: "h-full overflow-y-auto", style: { backgroundColor: 'var(--atomic-neutral-light)' } }, index.h(QuickviewSidebar, { words: this.words, i18n: this.bindings.i18n, highlightKeywords: this.highlightKeywords, onHighlightKeywords: (highlight) => (this.highlightKeywords = highlight), minimized: this.minimizeSidebar, onMinimize: (minimize) => (this.minimizeSidebar = minimize) })), index.h("div", { class: "overflow-auto relative" }, index.h(QuickviewIframe, { logger: this.logger, src: this.quickviewSrc, sandbox: this.sandbox, uniqueIdentifier: this.quickviewUniqueIdentifier, content: this.content, onSetIframeRef: async (ref) => {
                this.iframeRef = ref;
    ...
    renderFooter() {
        return (index.h("div", { slot: "footer", class: "flex items-center gap-2" }, index.h(button.Button, { class: "p-2", style: "square-neutral", onClick: () => { var _a; return (_a = this.previousQuickview) === null || _a === void 0 ? void 0 : _a.emit(); }, text: this.bindings.i18n.t('quickview-previous') }), index.h("p", { class: "text-center" }, this.bindings.i18n.t('showing-results-of', {
            first: this.current,
            total: this.total,
        })), index.h(button.Button, { class: "p-2", style: "square-neutral", onClick: () => { var _a; return (_a = this.nextQuickview) === null || _a === void 0 ? void 0 : _a.emit(); }, text: this.bindings.i18n.t('quickview-next') })));
    }
    onClose() {
        this.content = undefined;
        this.result = undefined;
...
AtomicQuickviewModal.style = AtomicQuickviewModalStyle0;

exports.atomic_quickview_modal = AtomicQuickviewModal;

//# sourceMappingURL=atomic-quickview-modal.cjs.entry.js.map