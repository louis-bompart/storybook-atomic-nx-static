...
// src/features/recommendation/recommendation-state.ts
var getRecommendationInitialState = () => ({
  duration: 0,
  error: null,
  isLoading: false,
  id: "Recommendation",
...
// src/api/analytics/recommendations-analytics.ts
var RecommendationAnalyticsProvider = class extends BaseAnalyticsProvider {
  getPipeline() {
  ...
  mapResultsToAnalyticsDocument() {
    return this.state.recommendation?.recommendations.map((r2) => ({
      documentUri: r2.uri,
      documentUriHash: r2.raw.urihash
    }));
  }
...
var partialRecommendationInformation = (result, state) => {
  const resultIndex = state.recommendation?.recommendations.findIndex(
...
// src/features/recommendation/recommendation-analytics-actions.ts
var logRecommendationUpdate = () => makeAnalyticsAction(
...
var recommendationInterfaceLoad = () => ({
  actionCause: "recommendationInterfaceLoad" /* recommendationInterfaceLoad */
});
var logRecommendationOpen = (result) => makeAnalyticsAction({
...
// src/features/recommendation/recommendation-actions.ts
var setRecommendationId = createAction(
  "recommendation/set",
  (payload) => validatePayload(payload, {
    id: requiredNonEmptyString
  })
);
var getRecommendations = createAsyncThunk(
  "recommendation/get",
...
var buildRecommendationRequest = async (s) => ({
...
var buildAnalyticsSection = async (state, action) => {
...
// src/features/recommendation/recommendation-slice.ts
var recommendationReducer = createReducer(
...
// src/app/recommendation-engine/recommendation-engine-configuration.ts
var recommendationEngineConfigurationSchema = new Schema({
...
// src/app/recommendation-engine/recommendation-engine.ts
var recommendationEngineReducers = {
  debug: debugReducer,
...
function buildRecommendationEngine(options) {
  ...
  const { pipeline, searchHub, timezone: timezone2, locale } = options.configuration;
...
var loadCollection = createAsyncThunk(
  ...
  async (collectionId, { getState, rejectWithValue, extra: { apiClient } }) => {
    const state = getState();
    const sharedWithSearchRequest = state.configuration.analytics.analyticsMode === "legacy" ? await buildSearchAndFoldingLoadCollectionRequest(state) : await buildSearchAndFoldingLoadCollectionRequest2(state);
...
// src/features/recommendation/recommendation-actions-loader.ts
function loadRecommendationActions(engine) {
  engine.addReducers({ recommendation: recommendationReducer });
  return {
    getRecommendations,
    setRecommendationId
  };
...
// src/features/recommendation/recommendation-click-analytics-actions-loader.ts
...
var buildSearchRequest = async (state, eventDescription) => {
  ...
  const sharedWithFoldingRequest = state.configuration.analytics.analyticsMode === "legacy" ? await buildSearchAndFoldingLoadCollectionRequest(
    state,
    eventDescription
  ) : await buildSearchAndFoldingLoadCollectionRequest2(
    state,
    eventDescription
  );
...
// src/controllers/recommendation/headless-recommendation.ts
...
// src/controllers/recommendation/result-list/headless-recommendation-interactive-result.ts
...
export { EcommerceDefaultFieldsToInclude as E, loadSearchConfigurationActions as a, buildRecommendationList as b, loadFieldActions as c, buildRecommendationEngine as d, loadConfigurationActions as e, buildInteractiveResult as f, getOrganizationEndpoints as g, loadRecommendationActions as l };

//# sourceMappingURL=headless.esm-c65bdd66.js.map