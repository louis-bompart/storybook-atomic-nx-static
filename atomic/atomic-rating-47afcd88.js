...
const Rating = (props) => {
    ...
    const width = ((props.numberOfActiveIcons / props.numberOfTotalIcons) * 100).toString() +
        '%';
    const iconSize = `${(_a = props.iconSize) !== null && _a !== void 0 ? _a : 0.75}rem`;
    const renderIcon = (active) => {
        return (h("atomic-icon", { icon: props.icon, class: `shrink-0 ${active ? 'icon-active' : 'icon-inactive'}`, style: { width: iconSize, height: iconSize } }));
    };
    ...
    return (h("div", { class: "relative w-max", part: "value-rating", role: "img", "aria-label": props.i18n.t('stars', {
            count: props.numberOfActiveIcons,
            max: props.numberOfTotalIcons,
        }) },
        h("div", { class: "z-0 flex gap-0.5" }, emptyIconDisplay()),
        h("div", { class: "absolute left-0 top-0 z-1 flex gap-0.5 overflow-hidden", style: { width } }, filledIconDisplay())));
};
const computeNumberOfStars = (value, field) => {
    ...
    if (Number.isNaN(valueAsNumber)) {
        throw new Error(`Could not parse "${value}" from field "${field}" as a number.`);
    }
    return valueAsNumber;
};

export { Rating as R, Star as S, computeNumberOfStars as c };

//# sourceMappingURL=atomic-rating-47afcd88.js.map